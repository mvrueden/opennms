...
GIT_BRANCH=${bamboo.repository.branch.name:-$(git branch | grep \* | cut -d' ' -f2 )}
...
RELEASE_BUILD_NUM=${bamboo.buildNumber:-1}
...
echo "export RELEASE_BUILD_NUM=${bamboo.buildNumber:-1}" >> $BASH_ENV
