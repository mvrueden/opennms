...
GIT_BRANCH=${bambooRepositoryBranchName:-$(git branch | grep \* | cut -d' ' -f2 )}
...
RELEASE_BUILD_NUM=${bambooBuildNumber:-1}
...
echo "export RELEASE_BUILD_NUM=${bambooBuildNumber:-1}" >> $BASH_ENV
